...
def generate_with_multiple_input(messages: List[Dict],
    ...
    # Classes Pydantic devem ser passadas diretamente em response_format.
    # Nesse caso, parse() valida a resposta e preenche message.parsed.
    response_format = payload.get("response_format")
    if isinstance(response_format, type) and issubclass(response_format, BaseModel):
        response = client.chat.completions.parse(**payload)
    else:
        response = client.chat.completions.create(**payload)

    try:
        message = response.choices[-1].message
        output_dict = {'role': message.role, 'content': message.content}
        if getattr(message, "parsed", None) is not None:
            output_dict["parsed"] = message.parsed
    except Exception as e:
        raise Exception(f"Failed to get correct output dict. Please try again. Error: {e}")
